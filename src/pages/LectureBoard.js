...
const LectureBoard = () => {
    ...
    useEffect(() => {
        ...
        setLastSelectedCategory(categoryMapping[subject]);
        setCurrentPage(page);
        
        const fetchInitialData = async () => {
            const data = await fetchLectures(categoryMapping[subject], '', 'TOTAL', page);
    ...
    const resetCategories = async () => {
        ...
        setSearchWord('');
        setLastSelectedCategory(categoryMapping[subject]);
        const data = await fetchLectures(categoryMapping[subject]);
        setLectures(data.post_list);
        setTotalPageCount(data.total_page_count);
        setCurrentPage(1);
    };
    ...
    const handleSearch = async (keyword) => {
        ...
        setLastSelectedCategory(categoryMapping[currentCategory]);
        const data = await fetchLectures(categoryMapping[currentCategory], keyword, searchFilterMapping[filterCondition]);
        setLectures(data.post_list);
        setTotalPageCount(data.total_page_count);
        setCurrentPage(1);
    };
    ...
    const handleCategoryClick = async (cat) => {
        ...
        setSelectedSubCategory(null);
        setSearchWord('');
        setLastSelectedCategory(categoryMapping[cat]);
        const data = await fetchLectures(categoryMapping[cat]);
        setLectures(data.post_list);
        setTotalPageCount(data.total_page_count);
        setCurrentPage(1);
    };
    ...
    const handleSubCategoryClick = async (subCat) => {
        ...
        setSearchWord('');
        setLastSelectedCategory(categoryMapping[subCat]);
        const data = await fetchLectures(categoryMapping[subCat]);
        setLectures(data.post_list);
        setTotalPageCount(data.total_page_count);
        setCurrentPage(1);
    };
