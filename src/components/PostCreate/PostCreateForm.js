...
import { createPost, modifyPost } from "../../apifetchers/fetcher";
...
const PostCreateForm = ({ initialData = null, postId, onClose }) => {
  ...
  const [files, setFiles] = useState(initialData ? initialData.files : []);
  ...
  useEffect(() => {
    if (initialData) {
      const initialCategoryKey = Object.keys(categoryMapping).find(key => categoryMapping[key] === initialData.category);
      setCategory(initialCategoryKey);
    }
  }, [initialData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      let response;
      if (postId) {
        response = await modifyPost(postId, formData);
      } else {
  ...
  return (
    <MainContainer>
      ...
      <Wrapper>
        <TitleWrapper>
          <Typography size="light_h1" color="black_gray">{postId ? '게시글 수정' : '게시글 생성'}</Typography>
        </TitleWrapper>
        ...
        <SelectCategoryWrapper>
          <SelectCategorycontainer>
            ...
            <PostCreateDropdown onCategorySelect={handleCategorySelect} initialCategory={category} />
        ...
        <InputContentWrapper>
          ...
          <InputContentSecondRow>
            <CkeditorBox setContent={handleContentChange} initialData={content} />
